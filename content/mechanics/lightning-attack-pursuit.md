...
values:
  - label: "Pursuit limit (Lightning Attack)"
    value: "6 per combo (datamined PursuitLimitLightningAttack = 6)"
...
counteredBy:
  - "Pursuit cap (6 Lightning Attacks per combo)"
  - "Combo scaling (each chase hit returns less)"
  - "Vanish / tech on the chase"
summary: "The chase mechanic that extends combos on a launched or knocked-away opponent. The datamine caps the Lightning Attack pursuit at 6 per combo (PursuitLimitLightningAttack = 6) — the standard ceiling on how many times you can re-chase before the combo must end. Chase attacks also continue wall/ground-slam combos. Subject to combo scaling, so each additional pursuit hit returns less."
asOfVersion: "v2.2 (2026-05-26 update)"
asOfDate: "2026-05-26"
lastVerified: "2026-06-10"
confidence: "community"
sources:
  - "data-mined/sparkingzerometa.com datamine (PursuitLimitLightningAttack = 6)"
...
- **Pursuit limit: 6 per combo.** The datamine exposes a hard cap — **`PursuitLimitLightningAttack = 6`** — meaning a Lightning Attack can re-chase a launched opponent up to **6 times** within a single combo before the pursuit must stop. This is the **standard** ceiling that keeps chase loops from running forever.
...
- **Combo scaling applies.** Every pursuit hit is still subject to the aggressive diminishing-returns curve, so the **6th** Lightning Attack in a chain does far less than the first — chasing all the way to the cap is not always optimal damage. See [[health-and-damage|combo scaling]].
- The opponent can **tech or [[vanish-z-counter|Vanish]]** out of the chase at the appropriate window, so pursuit is not guaranteed for all 6 reps against a competent defender.
